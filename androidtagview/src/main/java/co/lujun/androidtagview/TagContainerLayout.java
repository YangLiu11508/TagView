...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private void onAddTag(String text, Bitmap bitmap, int position) {
        if (position < 0 || position > mChildViews.size()){
            throw new RuntimeException("Illegal position!");
        }
        TagView tagView = new TagView(getContext(), text);
        initTagView(tagView);
        tagView.setBitmap(bitmap);
        mChildViews.add(position, tagView);
        if (position < mChildViews.size()){
            for (int i = position; i < mChildViews.size(); i++) {
                mChildViews.get(i).setTag(i);
    ...
    private void onAddTag(String text, Bitmap bitmap,int tagTextDirection, int position) {
        if (position < 0 || position > mChildViews.size()){
            throw new RuntimeException("Illegal position!");
        }
        TagView tagView = new TagView(getContext(), text);
        initTagView(tagView);
        tagView.setBitmap(bitmap);
        tagView.setTextDirection(tagTextDirection);
        mChildViews.add(position, tagView);
        if (position < mChildViews.size()){
            for (int i = position; i < mChildViews.size(); i++) {
                mChildViews.get(i).setTag(i);
    ...
    public void addTag(String text, Bitmap bitmap){
        onAddTag(text, bitmap,mChildViews.size());
        postInvalidate();
    }
